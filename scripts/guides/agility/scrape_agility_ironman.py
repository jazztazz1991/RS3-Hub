...
def scrape_guide():
    url = "https://runescape.wiki/w/Ironman_Mode/Strategies/Agility"
    ...
    try:
        ...
        print(f"Error scraping Agility (ironman): {e}")
        return

    # Output path
    # Go up 3 levels from scripts/guides/{skill}/script.py to root/client/src/data/guides/{skill}
    base_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(base_dir, '..', '..', '..', 'client', 'src', 'data', 'guides', 'agility')
    output_path = os.path.join(output_dir, 'agilityIronman.json')
