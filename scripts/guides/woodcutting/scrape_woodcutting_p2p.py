...
def scrape_woodcutting_p2p():
    url = "https://runescape.wiki/w/Pay-to-play_Woodcutting_training"
    methods = []
    ...
    try:
        ...
        # In Woodcutting guide there should be a table with "Level", "Method", "XP/hour"
        target_table = None
        for table in tables:
        ...
        if target_table:
            ...
            for row in rows:
                ...
                try:
                    ...
                    # Clean up method name (remove [edit] etc)
                    method_name = re.sub(r'\[.*?\]', '', method_text).strip()
                    method_name = re.sub(r'\s+', ' ', method_name) # Fix newlines to spaces
        ...
        print(f"Error scraping Woodcutting P2P: {e}")

    # Output to distinct P2P file
    output_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'client', 'src', 'data', 'guides', 'woodcutting', 'woodcuttingP2P.json')
